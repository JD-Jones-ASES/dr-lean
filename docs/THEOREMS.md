...
| `DittertRybin.uniform_maximum_order_three` | K=3 on every M,N>=3. | Pending; complete three-row family, four-row N>=960, all min(M,N)>=10 and five-through-nine-row infinite strips proved; see [range map](ORDER-THREE-RANGES.md) |
| `DittertRybin.uniform_maximum_four_rows` | K=4 on every 4 by N, N>=4, and transpose. | Pending; actual averaging kernel and explicit concentration-region rigidity proved; see [foundation map](RECTANGULAR-FOUNDATIONS.md) |
