...
/-! The exact centered-deletion estimate with the transition row budget.
The original and retained row marginals are kept distinct. -/
namespace DittertRybin
open scoped BigOperators

theorem deleted_row_sq_scaled_le {m : ℕ} (r tau : Fin m → ℝ) (w : ℝ)
    (htau : ∀ i,0≤tau i) (hs : ∑ i,tau i=w) (hw : w<1) :
    (∑ i,(1-(m:ℝ)*((r i-tau i)/(1-w)))^2)*(1-w)^2≤
      2*(∑ i,((m:ℝ)*r i-1)^2)+2*(m:ℝ)^2*w^2 := by
...
theorem transition_deleted_row_sq_lt {m : ℕ} (hm : 1≤m)
    (r tau : Fin m → ℝ) (w : ℝ) (htau : ∀ i,0≤tau i)
    (hs : ∑ i,tau i=w) (hw : (m:ℝ)*w≤1/16)
    (hr : (∑ i,((m:ℝ)*r i-1)^2)<1/100) :
...
theorem transition_kept_row_sq_lt {m n : ℕ} (hm : 1≤m) {P : Board m n}
  ...
  have hs : (∑ i,tau i)=1-totalMass (keepColumns P S) := by
    ...
    rw [hP.2]
  have h := transition_deleted_row_sq_lt hm (rowSum P) tau
